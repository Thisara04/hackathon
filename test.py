...
def fetch_financial_data():
    ...
    # ── 1. Exchange rates — USD and EUR only ──
    try:
        url = (f"https://api.exchangeratesapi.io/v1/latest"
               f"?access_key={EXCHANGE_RATE_API_KEY}&base=EUR&symbols=LKR,USD")
        ...
        result["fx"] = {"USD_LKR": 309.50, "EUR_LKR": 335.20, "date": "N/A"}

    # ── 2. Gold & Silver — metals.live returns a list ──
    try:
        g = requests.get("https://api.metals.live/v1/spot", timeout=5).json()
        if isinstance(g, list):
            metals = {item["metal"]: item["price"] for item in g if "metal" in item and "price" in item}
            result["gold_usd"]   = round(metals.get("gold",   3320.00), 2)
            result["silver_usd"] = round(metals.get("silver",   32.50), 2)
        else:
...
# ═══════════════════════════════════════════════════════════
    ...
    fx_pairs = [
        ("🇺🇸 USD / LKR", fx.get("USD_LKR")),
        ("🇪🇺 EUR / LKR", fx.get("EUR_LKR")),
    ]
    fx_cols = st.columns(2)
    for i, (label, val) in enumerate(fx_pairs):
